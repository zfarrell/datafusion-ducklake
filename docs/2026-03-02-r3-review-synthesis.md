...
#### R3F-001: Missing `ducklake_table_column_stats` causes DuckDB crash
...
#### R3F-002: `register_dml_files` omits `row_id_start` and `table_stats` for new data files
...
#### R3F-003: MERGE execution has no orphaned file cleanup on metadata commit failure
...
#### R3F-004: Date32/Date64 inlined data roundtrip broken — silent data loss
...
#### R3F-005: Timestamp inlined data roundtrip broken — silent data loss
...
#### R3F-006: PG/MySQL writers missing multiple fixes applied only to SQLite
...
#### R3F-008: `quote_identifier` not applied consistently across inlined data paths
...
#### R3F-009: `.unwrap()` on downcasts in non-test production code
...
#### R3F-010: Unchecked `as` casts across DML execs and supporting code
...
#### R3F-011: `next_catalog_id` and `next_file_id` never populated in snapshots
...
#### R3F-012: Timestamp non-UTC timezone silently replaced with UTC on roundtrip
...
#### R3F-013: DML snapshots missing `snapshot_changes` records
...
#### R3F-014: Missing `created_schema` change tracking
...
#### R3F-015: `batches_to_strings` semantic divergence across test files
...
#### R3F-016: Read-only write test silently passes if write succeeds
...
#### R3F-017: `set_data_path` is not atomic (DELETE + INSERT without transaction)
...
#### R3F-018: `table_deletions` returns wrong column order for reordered full projections
...
#### R3F-019: `get_table_structure` is not snapshot-aware in table functions
...
#### R3F-020: ORDER BY ALL rewriting incorrect for multi-line SQL in SLT runner
...
#### R3F-021: Incomplete migration to shared test helpers
...
#### R3F-022: Missing test coverage areas
...
#### R3F-023: `statement error` → `statement ok` conversion visibility
...
#### R3F-024: `test_double_slash_in_various_positions` test failure
...
#### R3F-025: Dead variables with underscore prefix (unused allocations)
...
#### R3F-026: `Date64` → `"date"` → `Date32` lossy roundtrip in type system
...
#### R3F-027: `Interval` variant lost on roundtrip
...
#### R3F-028: Extra columns in catalog tables vs DuckDB reference
...
#### R3F-029: `parse_decimal` silently ignores trailing garbage after closing parenthesis
...
#### R3F-030: `parse_string_to_array` fallback silently downcasts unsupported types to string
...
#### R3F-031: Duplicated Parquet write + delete-file boilerplate across DML execs
...
#### R3F-032: Empty snapshots created when DML affects zero rows
...
#### R3F-033: MERGE source rows can match multiple target rows without error
...
#### R3F-034: `delete_count` metadata tracks delta, not total positions in delete file
...
#### R3F-035: `CoalescePartitionsExec` wrapping fragile across `with_new_children`
...
#### R3F-036: `changes_made` format doesn't escape quotes in schema/table names
...
#### R3F-037: `rewrite_unqualified_tables` is a no-op wasting allocations
...
#### R3F-038: DuckDB error handling inconsistency for missing `data_path`
...
#### R3F-039: Bare `decimal`/`numeric` without parameters produces misleading error
...
#### R3F-040: `information_schema.rs` `table_exist()` allocates on every call
...
#### R3F-041: `Arc::clone` vs `.clone()` inconsistency on Arc types
...
#### R3F-042: `information_schema.rs` TableProvider boilerplate
...
#### R3F-043: `get_int_column()` duplicated in 3 test files
...
#### R3F-044: Third `convert_batch_to_strings` variant in `hybrid_asyncdb.rs`
...
#### R3F-045: `test_utils` feature gate could break without `metadata-duckdb`
...
#### R3F-046: `WITH ... INSERT` not caught by SLT write statement detection
...
#### R3F-047: Transaction-mode DuckDB formatting may differ from DataFusion
...
#### R3F-048: Decimal type string spacing difference
...
#### R3F-049: Extra non-standard tables in DDL (informational)
...
#### R3F-050: 5/7 `sql_write_tests` are `#[ignore]`d (informational)
...
|----------|-------|-----------|
| P0 | 3 | Table column stats crash, DML row_id_start, MERGE orphan cleanup |
| P1 | 9 | Inlined data roundtrip, PG/MySQL fix parity, schema_version, quoting, casts, panics |
| P2 | 16 | Snapshot changes, test harness, type lossy roundtrips, atomicity gaps |
| P3 | 22 | Code quality, minor consistency, informational items |
| **Total** | **50** | |
