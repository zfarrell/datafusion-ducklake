...
#### R4-S-001: Inline data cleared before durable Parquet replacement
- **Source**: codex (CX-001)
...
#### R4-S-002: Inline flush writes to `t{table_id}/` but table path is `table_name/`
...
#### R4-S-003: Partitioned commit non-atomic
...
#### R4-S-004: `register_dml_files` doesn't update snapshot `next_file_id`
...
#### R4-S-005: DML data files missing `register_column_stats`
...
#### R4-S-006: PG/MySQL `register_dml_files` missing `row_id_start` and `table_stats`
...
#### R4-S-007: `end_table_files` (Replace mode) doesn't reset `table_stats`
...
#### R4-S-008: UPDATE/MERGE `snapshot_changes` use non-standard tokens
...
#### R4-S-009: Delete file `file_path` column uses relative catalog path
...
#### R4-S-010: NULL filter predicate incorrectly treated as match in DELETE/UPDATE
- **Source**: codex (CX-005)
...
#### R4-S-011: UPDATE/MERGE skip NOT NULL constraint validation
- **Source**: codex (CX-007)
...
#### R4-S-012: LIMIT pushed into Parquet scan before DeleteFilterExec
- **Source**: codex (CX-041)
...
#### R4-S-013: `record_count` never decremented after DELETE
...
#### R4-S-014: Drop/create operations missing existence validation
...
#### R4-S-015: `rename_table`/`rename_view` allow duplicate active names
- **Source**: codex (CX-022)
...
#### R4-S-016: `record_snapshot_changes` failure after successful DML commit
...
#### R4-S-017: `finish()` cleanup deletes already-committed metadata file
- **Source**: codex (CX-004)
...
#### R4-S-018: Checked write transactions TOCTOU in PG/MySQL
- **Source**: codex (CX-023)
...
#### R4-S-019: `list_all_columns` violates snapshot isolation
- **Source**: codex (CX-040)
...
#### R4-S-020: Provider errors silently swallowed
...
#### R4-S-021: Residual unsafe `as` casts in non-DML files
...
#### R4-S-022: Struct field names not escaped in `arrow_to_ducklake_type`
- **Source**: codex (CX-061)
...
#### R4-S-023: Inlined data table uses TEXT for all columns and non-standard naming
...
#### R4-S-024: Data/delete file naming and field_id divergence from DuckDB
...
#### R4-S-025: `parse_decimal` accepts malformed input
- **Source**: codex (CX-060)
...
#### R4-S-026: Projection index bounds not checked in CDC functions
- **Source**: codex (CX-062)
...
#### R4-S-027: CDC projection analysis duplicated
...
#### R4-S-028: Virtual column filtering inconsistency in test helpers
...
#### R4-S-029: Decimal128/float formatting divergence between test paths
...
#### R4-S-030: `DuckDbConn` struct duplicated across 10 test files
...
#### R4-S-032: No unit tests for `is_write_statement` routing function
...
#### R4-S-033: `convert_batch_to_strings` missing types fall through silently
...
#### R4-S-034: Lossy i64→f64 cast in compaction_functions
- **Source**: idiomatic (I-002)
- **Files**: `compaction_functions.rs:482`
- **Fix**: Add range guard for values above 2^53.

#### R4-S-035: Unchecked i64 overflow in virtual_column_exec
- **Source**: codex (CX-064)
- **Files**: `virtual_column_exec.rs:230-232,260-261`
- **Fix**: Use `checked_add()`.

#### R4-S-036: Pervasive `.map_err(DataFusionError::External(Box::new(e)))` boilerplate
...
#### R4-S-045: `assert_results_eq` doesn't show differing rows on count mismatch
- **Source**: test-harness (TH-017)
- **Files**: `tests/common/test_utils.rs`
- **Fix**: Print first few rows on row-count mismatch.

#### R4-S-046: Edge case handling in DuckDB provider
...
### Agent 1: DML Metadata Integrity (P0 + P1)
- **Findings**: R4-S-001, R4-S-002, R4-S-004, R4-S-005, R4-S-007, R4-S-013
- **Estimated effort**: L (aggregate)
- **Key files**: `table_writer.rs`, `metadata_writer_sqlite.rs`, `update_exec.rs`, `merge_exec.rs`, `delete_exec.rs`
- **Description**: Fix inline data flush ordering (P0), fix inline flush write path, update next_file_id in DML, add column stats to DML files, reset table_stats in Replace mode, decrement record_count after DELETE.

### Agent 2: DML Correctness (P1)
- **Findings**: R4-S-010, R4-S-011, R4-S-012
- **Estimated effort**: M
- **Key files**: `delete_exec.rs`, `update_exec.rs`, `merge_exec.rs`, `table.rs`
- **Description**: Fix NULL filter predicate handling, add NOT NULL validation in UPDATE/MERGE, fix LIMIT pushdown for files with deletes.

### Agent 3: Interop Format Fixes (P1)
- **Findings**: R4-S-008, R4-S-009
- **Estimated effort**: S
- **Key files**: `update_exec.rs`, `merge_exec.rs`, `delete_exec.rs`
- **Description**: Fix snapshot_changes tokens for UPDATE/MERGE, resolve full path for delete file file_path column.

### Agent 4: PG/MySQL Fix Parity (P1)
- **Findings**: R4-S-006
- **Estimated effort**: S
- **Key files**: `metadata_writer_postgres.rs`, `metadata_writer_mysql.rs`
- **Description**: Port R3F-002 fix (row_id_start + table_stats) to PG/MySQL register_dml_files.

### Agent 5: Atomicity + Validation (P1 + P2)
- **Findings**: R4-S-003, R4-S-014, R4-S-015, R4-S-016, R4-S-017
- **Estimated effort**: M
- **Key files**: `table_writer.rs`, all `metadata_writer_*.rs`
- **Description**: Wrap partitioned commit in transaction, add drop/rename validation, fix record_snapshot_changes failure handling, fix finish() cleanup logic.

### Agent 6: Code Quality + Safety (P2)
- **Findings**: R4-S-019, R4-S-020, R4-S-021, R4-S-022, R4-S-025, R4-S-026
- **Estimated effort**: M
- **Key files**: `metadata_provider_*.rs`, `table_deletions.rs`, `delete_filter.rs`, `table_functions.rs`, `types.rs`, `table.rs`
- **Description**: Fix list_all_columns snapshot isolation, propagate provider errors, replace residual `as` casts, escape struct field names, fix parse_decimal, add projection bounds checks.

### Agent 7: Interop Convention Alignment (P2)
- **Findings**: R4-S-023, R4-S-024, R4-S-027
- **Estimated effort**: S
- **Key files**: `metadata_writer_sqlite.rs`, `table_writer.rs`, `table.rs`, `table_changes.rs`, `table_deletions.rs`
- **Description**: Fix inlined data table types/naming, add ducklake- prefix to data files, add field_ids to delete file schema, extract shared CDC projection analysis.

### Agent 8: Test Infrastructure (P2)
- **Findings**: R4-S-028, R4-S-029, R4-S-030, R4-S-031, R4-S-032, R4-S-033
- **Estimated effort**: M
- **Key files**: `tests/merge_tests.rs`, `tests/cross_engine_alter_tests.rs`, `tests/hybrid_asyncdb.rs`, `tests/common/test_utils.rs`, `tests/adversarial_catalog_tests.rs`, `tests/sqllogictest_runner.rs`
- **Description**: Fix virtual column filtering in tests, unify Decimal128/float formatting, extract DuckDbConn, fix tautological assertions, add is_write_statement tests, fix missing type handling.
