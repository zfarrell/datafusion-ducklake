...
pub fn normalize_value(s: &str) -> String {
    ...
    // Normalize values that look like floats — containing '.' or scientific notation.
...
/// Cross-row "either-side" normalization used by `assert_results_eq`.
///
/// `normalize_value` is conservative: it only normalizes strings that look
/// like floats (contain '.' or 'e'/'E'). That breaks DuckDB-vs-DataFusion
/// parity assertions where one engine prints a `DOUBLE` column as `"20"`
/// (DataFusion's compact representation) and the other prints it as
/// `"20.000000"` (DuckDB's). Without this hook those values normalize to
/// `"20"` vs `"20.000000"` and the test fails despite being value-equal.
///
/// This helper looks at BOTH sides: if either side already looks like a
/// decimal float, normalize the OTHER side through `f64::parse` too. That
/// preserves the strict integer/float distinction in the common case but
/// still equates `"20"` with `"20.000000"` when the partner value
/// disambiguates the intent.
fn normalize_pair(exp: &str, act: &str) -> (String, String) {
    let exp_norm = normalize_value(exp);
    let act_norm = normalize_value(act);
    let exp_looks_float =
        exp.contains('.') || exp.contains('e') || exp.contains('E');
    let act_looks_float =
        act.contains('.') || act.contains('e') || act.contains('E');
    if exp_looks_float != act_looks_float {
        // One side has decimal/exponent notation, the other doesn't.
        // Try to coerce the non-decimal side into the same numeric shape.
        let exp_final = if !exp_looks_float
            && let Ok(f) = exp.parse::<f64>()
        {
            format!("{:.6}", f)
        } else {
            exp_norm
        };
        let act_final = if !act_looks_float
            && let Ok(f) = act.parse::<f64>()
        {
            format!("{:.6}", f)
        } else {
            act_norm
...
pub fn assert_results_eq(scenario: &str, expected: &[Vec<String>], actual: &[Vec<String>]) {
    ...
    for (i, (exp_row, act_row)) in expected.iter().zip(actual.iter()).enumerate() {
        ...
        for (j, (exp_val, act_val)) in exp_row.iter().zip(act_row.iter()).enumerate() {
            let (exp_norm, act_norm) = normalize_pair(exp_val, act_val);
            assert_eq!(
                exp_norm, act_norm,
