...
async fn test_query_real_parquet_files() {
    ...
    // Verify schema. 3 real columns. DuckLake virtual columns are gated
    // behind `DuckLakeCatalog::with_row_lineage(true)` (ticket #22).
    assert_eq!(
        batch.num_columns(),
        3,
...
async fn test_get_inlined_data_with_data() {
    ...
    assert_eq!(data[0].column_names.as_ref(), &vec!["name".to_string(), "age".to_string()]);
    assert_eq!(
        data[0].values,
