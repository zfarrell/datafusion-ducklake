//! Shared `MetadataWriter` implementations for sqlx-based catalog backends.
//!
//! ## What this module provides
//!
//! A set of `macro_rules!` macros that each expand to a chunk of
//! `MetadataWriter` (or supporting) functionality, parameterised on the
//! backend's sqlx [`Transaction`][sqlx::Transaction] type and a
//! [`SqlDialect`][crate::dialect::SqlDialect] instance. The same macro
//! invocations produce the SQLite, PostgreSQL, and MySQL writers — only the
//! transaction type and dialect change.
//!
//! ## Macros
//!
//! - [`impl_recompute_table_column_stats!`] — table-level stat aggregation helper.
//! - [`impl_writer_query_ops!`] — read-side query helpers used by the writer
//!   (config lookups, schema/table existence, partition column reads, …).
//! - [`impl_writer_file_ops!`] — INSERT/UPSERT for data and delete files,
//!   per-file stats, and snapshot bookkeeping.
//! - [`impl_writer_ddl_ops!`] — CREATE SCHEMA / CREATE TABLE / CREATE VIEW /
//!   ALTER TABLE plus their DDL snapshot rows.
//! - [`impl_writer_drop_inner!`] / [`impl_writer_drop_ops!`] — DROP SCHEMA /
//!   DROP TABLE / DROP VIEW soft-deletion via `end_snapshot`.
//! - [`create_ddl_snapshot!`] — internal helper used by the DDL macros to
//!   allocate a new snapshot id with the appropriate `FOR UPDATE` lock.
//!
//! ## Contract for backend authors
//!
//! Each backend supplies:
//!
//! 1. A struct (e.g. `SqliteMetadataWriter`) that holds a `sqlx::Pool<Db>`.
//! 2. An implementation of [`SqlDialect`][crate::dialect::SqlDialect] for its
//!    target SQL dialect.
//! 3. A small amount of backend-specific glue: `new(...)`, `pool()`, optional
//!    busy-retry, and any sequence/id allocation that doesn't fit the
//!    `next_id_sql` shape (MySQL provides its own `next_sequence_id` since it
//!    has no sequences).
//!
//! It then invokes the macros listed above inside the appropriate `impl`
//! blocks. The macros call `SqlDialect` trait methods to emit syntax that
//! differs between backends (parameter placeholders, `RETURNING`,
//! `FOR UPDATE`, upsert syntax, boolean literals, …).
//!
//! ## Worked example
//!
//! See [`metadata_writer_sqlite.rs`][crate::metadata_writer_sqlite] for the
//! canonical reference invocation. The Postgres and MySQL writers
//! (`metadata_writer_postgres.rs`, `metadata_writer_mysql.rs`) follow the same
//! pattern with their respective dialect/transaction types.
//!
//! ## Invariants
//!
//! - Macros never embed user input directly into SQL. All identifiers come
//!   from compile-time constants or the (validated) catalog schema. The only
//!   runtime values reach the database via bound parameters generated by
//!   `SqlDialect::ph` / `uuid_ph`.
//! - Macro expansion is purely syntactic: each backend gets its own
//!   monomorphised copy. There is no `dyn SqlDialect` at the call sites — the
//!   dialect is a unit struct evaluated at compile time.
