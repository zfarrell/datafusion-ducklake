...
impl MetadataWriter for PostgresMetadataWriter {
    ...
    fn register_data_file(
        ...
        block_on(async {
            ...
            let stats_row = sqlx::query(
                "SELECT next_row_id FROM ducklake_table_stats WHERE table_id = $1 FOR UPDATE",
            )
            .bind(table_id)
            .fetch_optional(&mut *tx)
            .await?;
            ...
            let row = sqlx::query(
                "INSERT INTO ducklake_data_file (table_id, path, path_is_relative, file_size_bytes, footer_size, record_count, row_id_start, file_format, begin_snapshot)
                 VALUES ($1, $2, $3, $4, $5, $6, $7, 'parquet', $8) RETURNING data_file_id",
            )
            .bind(table_id)
            .bind(&file.path)
    ...
    fn register_dml_files(
        ...
        block_on(async {
            ...
            if total_net_new_deletions > 0 {
                sqlx::query(
                    "UPDATE ducklake_table_stats
                     SET record_count = GREATEST(0, COALESCE(record_count, 0) - $1)
                     WHERE table_id = $2",
                )
            ...
            for file in data_files {
                ...
                // R7-S-011: Use RETURNING to get data_file_id for column stats
                let row = sqlx::query(
                    "INSERT INTO ducklake_data_file (table_id, path, path_is_relative, file_size_bytes, footer_size, record_count, row_id_start, begin_snapshot)
                     VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING data_file_id",
                )
    ...
    fn register_delete_file(
        ...
        block_on(async {
            ...
            let row = sqlx::query(
                "INSERT INTO ducklake_delete_file (data_file_id, table_id, path, path_is_relative, file_size_bytes, footer_size, delete_count, format, begin_snapshot)
                 VALUES ($1, $2, $3, $4, $5, $6, $7, 'parquet', $8) RETURNING delete_file_id",
            )
            .bind(file.data_file_id)
            .bind(table_id)
            .bind(&file.path)
