...
- **Resolution**: **72 FIXED**, 1 false positive, 4 skipped/deferred
...
Eight fix agents resolved **72 of 77** findings. 6 findings were verified as already correct (no code change needed). 4 findings were skipped/deferred. 1 was a false positive.

### Fix Agents

| Agent | Commit | Findings Fixed | Notes |
|-------|--------|----------------|-------|
| fix-backend-parity | `15b746f` | R5-S-013,018,027,029,044,047,052,053,059,065,066,071,072,078,079 (15) | R5-S-052 already fixed prior |
| fix-table-functions | `15b746f` | R5-S-009,028,030,038,039,048,055,062,075 (9) | R5-S-062 verified safe, no change |
| fix-dml-robustness | `fc2f5da` | R5-S-006,021,034,035,037,045,046,049,050,057,058,064,076 (13) | R5-S-057 already existed |
| fix-metadata-correctness | various | R5-S-001,002,007,008,025,026,036,073 (8) | R5-S-036 verified working, R5-S-056 false positive, R5-S-060/074 skipped |
| fix-cross-engine | `fc2f5da`,`7c685bd` | R5-S-011,067,077,081 (4) | 11 new cross-engine tests |
| fix-write-safety | various | R5-S-003,004,005,019,020,022,024,061 (8) | R5-S-022/024 verified+documented |
| fix-interop-types | `84d51ff` | R5-S-012,014,015,016,017,054,063 (7) | R5-S-063 cosmetic, no change |
| fix-test-infrastructure | `08f55a9` | R5-S-010,023,031,032,033,040,041,042,043,069,070,080 (12) | 4 new tests |

### Verified as Already Correct (no code change needed)
- **R5-S-022**: Transaction semantics verified and documented
- **R5-S-024**: Transaction semantics verified and documented
- **R5-S-036**: Filter pushdown verified working via DataFusion optimizer
- **R5-S-052**: Already fixed in prior round
- **R5-S-057**: Guard already existed
- **R5-S-062**: SQL interpolation already safe
- **R5-S-063**: UUID v4 vs v7 cosmetic only
- **R5-S-075**: join_paths limitation documented

### Skipped/Deferred
- **R5-S-051**: Arc::clone style consistency — too broad/mechanical
- **R5-S-068**: Parquet v2 vs v1 — no functional issue, no fix needed
- **R5-S-060**: deregister_schema cascade — requires MetadataWriter trait API change, skipped
- **R5-S-074**: get_table_row_count no transaction — minimal impact, skipped

### False Positive
- **R5-S-056**: Code already uses char-based iteration
...
#### R5-S-001: Lexicographic MIN/MAX on VARCHAR column stats — **[FIXED]** (fix-metadata-correctness)
- **Source**: correctness
...
#### R5-S-002: replace_table_files doesn't update table_stats after compaction — **[FIXED]** (fix-metadata-correctness)
...
#### R5-S-003: contains_null inconsistency in PG/MySQL ALTER TABLE Add COLUMN — **[FIXED]** (fix-write-safety)
...
#### R5-S-004: Inlining flush path silently loses existing inline rows — **[FIXED]** (fix-write-safety)
...
#### R5-S-005: Date32/Date64 partition pruning uses epoch-days, DuckDB uses ISO strings — **[FIXED]** (fix-write-safety)
...
#### R5-S-006: UPDATE can panic on invalid assignment column index — **[FIXED]** (`fc2f5da`, fix-dml-robustness)
- **Source**: codex (CX-008)
...
#### R5-S-007: Delete-delta boundary uses BETWEEN (inclusive) instead of strict lower bound — **[FIXED]** (fix-metadata-correctness)
- **Source**: codex (CX-013)
...
#### R5-S-008: statistics() sized to base columns, schema() returns full_schema with virtuals — **[FIXED]** (fix-metadata-correctness)
- **Source**: codex (CX-015)
...
#### R5-S-009: strip_prefix can mis-trim paths in ducklake_list_files — **[FIXED]** (`15b746f`, fix-table-functions)
- **Source**: codex (CX-016)
...
#### R5-S-010: Decimal128 negative sign loss when whole part is zero (test formatter) — **[FIXED]** (`08f55a9`, fix-test-infrastructure)
...
#### R5-S-011: Missing cross-engine tests for DML, ALTER TABLE, partitions, and complex types — **[FIXED]** (`fc2f5da`+`7c685bd`, fix-cross-engine, 11 new tests)
...
#### R5-S-012: Unknown types silently converted to string/Utf8 — **[FIXED]** (`84d51ff`, fix-interop-types)
...
#### R5-S-013: schema_version column type INTEGER in PG/MySQL, DuckDB uses BIGINT — **[FIXED]** (`15b746f`, fix-backend-parity)
...
#### R5-S-014: Inlined Date/Timestamp serialization uses epoch integers — **[FIXED]** (`84d51ff`, fix-interop-types)
...
#### R5-S-015: Inlined data flush fails for Decimal types — **[FIXED]** (`84d51ff`, fix-interop-types)
...
#### R5-S-016: Decimal column stats silently dropped (FixedLenByteArray as UTF-8) — **[FIXED]** (`84d51ff`, fix-interop-types)
...
#### R5-S-017: register_dml_files omits format column for delete files — **[FIXED]** (`84d51ff`, fix-interop-types)
...
#### R5-S-018: _df_change_tracking blind to DuckDB-side changes — **[FIXED]** (`15b746f`, fix-backend-parity)
...
#### R5-S-019: saturating_add silently clips row count to i64::MAX — **[FIXED]** (fix-write-safety)
...
#### R5-S-020: unwrap_or(i64::MAX) clips rowid overflow — **[FIXED]** (fix-write-safety)
...
#### R5-S-021: values_equal doesn't handle NaN for float join keys — **[FIXED]** (`fc2f5da`, fix-dml-robustness)
...
#### R5-S-022: Replace mode atomicity improvement — **[VERIFIED]** (transaction semantics verified and documented, fix-write-safety)
...
#### R5-S-023: normalize_value over-normalizes all numeric strings — **[FIXED]** (`08f55a9`, fix-test-infrastructure)
...
#### R5-S-024: commit_uploaded_files non-atomic in append mode — **[VERIFIED]** (transaction semantics verified and documented, fix-write-safety)
- **Source**: codex (CX-005)
...
#### R5-S-025: column_stats failure propagates after data file committed — **[FIXED]** (fix-metadata-correctness)
- **Source**: codex (CX-006)
...
#### R5-S-026: SQLite INSERT OR REPLACE erases snapshot_changes metadata — **[FIXED]** (fix-metadata-correctness)
- **Source**: codex (CX-009)
...
#### R5-S-027: MySQL ID allocation race-prone with MAX+1 — **[FIXED]** (`15b746f`, fix-backend-parity)
- **Source**: codex (CX-011)
...
#### R5-S-028: Inlined-table resolution ignores snapshot/version semantics — **[FIXED]** (`15b746f`, fix-table-functions)
- **Source**: codex (CX-012)
...
#### R5-S-029: DuckLakeSchema snapshot not refreshed after DDL writes — **[FIXED]** (`15b746f`, fix-backend-parity)
- **Source**: codex (CX-014)
...
#### R5-S-030: Table function dot-splitting breaks quoted identifiers — **[FIXED]** (`15b746f`, fix-table-functions)
...
#### R5-S-031: DuckDB decode error silently converted to "NULL" in tests — **[FIXED]** (`08f55a9`, fix-test-infrastructure)
- **Source**: codex (CX-018)
...
#### R5-S-032: Zero-row DML returns StatementComplete instead of count — **[FIXED]** (`08f55a9`, fix-test-infrastructure)
...
#### R5-S-033: rewrite_table_references does raw substring replacement — **[FIXED]** (`08f55a9`, fix-test-infrastructure)
- **Source**: codex (CX-020)
...
#### R5-S-034: UPDATE buffers all matched rows in memory — **[FIXED]** (`fc2f5da`, fix-dml-robustness)
- **Source**: codex (CX-021)
...
#### R5-S-035: MERGE uses O(N*M) nested loop join — **[FIXED]** (`fc2f5da`, fix-dml-robustness)
- **Source**: codex (CX-022)
...
#### R5-S-036: Filters advertised as Inexact but not pushed to Parquet scan — **[VERIFIED]** (filter pushdown works via DataFusion optimizer, fix-metadata-correctness)
- **Source**: codex (CX-028)
...
#### R5-S-037: CDC projection HashMap dedup breaks duplicate column projections — **[FIXED]** (`fc2f5da`, fix-dml-robustness)
- **Source**: codex (CX-030)
...
#### R5-S-038: Snapshot bounds only accept Int32/Int64 literals — **[FIXED]** (`15b746f`, fix-table-functions)
- **Source**: codex (CX-032)
...
#### R5-S-039: table_changes only emits Insert, never Delete — **[FIXED]** (`15b746f`, fix-table-functions)
- **Source**: codex (CX-033)
...
#### R5-S-040: Pre-1970 timestamp conversion can panic — **[FIXED]** (`08f55a9`, fix-test-infrastructure)
- **Source**: codex (CX-035)
...
#### R5-S-041: Virtual column stripping uses naive SQL string matching — **[FIXED]** (`08f55a9`, fix-test-infrastructure)
...
#### R5-S-042: Timestamp sub-second precision inconsistency between formatters — **[FIXED]** (`08f55a9`, fix-test-infrastructure)
...
#### R5-S-043: SLT preprocessor vacuous-test guard may be too weak — **[FIXED]** (`08f55a9`, fix-test-infrastructure)
...
|----|-------------|-------|--------|
| R5-S-044 | Duplicated `quote_identifier` function | `metadata_provider.rs:782`, `metadata_writer_validation.rs:67` | S | **[FIXED]** fix-backend-parity |
| R5-S-045 | Duplicated `make_*_count_schema()` across DML exec plans | `delete_exec.rs`, `update_exec.rs`, `merge_exec.rs`, `insert_exec.rs` | S | **[FIXED]** fix-dml-robustness |
| R5-S-046 | Manual type dispatch in `values_equal()` — Arrow eq kernel available | `merge_exec.rs` | S | **[FIXED]** fix-dml-robustness |
| R5-S-047 | Repetitive TableProvider boilerplate in information_schema.rs (6 near-identical impls) | `information_schema.rs` | M | **[FIXED]** fix-backend-parity |
| R5-S-048 | Inline helper lambdas (`to_string_array`, `to_i64_array`) duplicated in compaction_functions.rs | `compaction_functions.rs` | S | **[FIXED]** fix-table-functions |
| R5-S-049 | Unnecessary `positions_to_delete.clone()` | `delete_exec.rs:340` | S | **[FIXED]** fix-dml-robustness |
| R5-S-050 | Redundant `let batch = batch_result;` binding | `delete_exec.rs` | S | **[FIXED]** fix-dml-robustness |
| R5-S-051 | Inconsistent `Arc::clone(&x)` vs `x.clone()` style | Throughout codebase | S | **[SKIPPED]** too broad/mechanical |
| R5-S-052 | `.iter().map(\|r\| r.clone())` instead of `.into_iter()` | `metadata_provider_sqlite.rs` | S | **[VERIFIED]** already fixed prior |
| R5-S-053 | Public `pool` fields on metadata providers leak implementation | PG/MySQL/SQLite providers | S | **[FIXED]** fix-backend-parity |
| R5-S-054 | `DuckLakeEncryptionFactory` derives `Clone` but never cloned (security concern) | `encryption.rs` | S | **[FIXED]** fix-interop-types |
| R5-S-055 | `keep_indices.clone()` may be unnecessary | `table_deletions.rs:702` | S | **[FIXED]** fix-table-functions |

#### Correctness Edge Cases

| ID | Description | Files | Effort |
|----|-------------|-------|--------|
| R5-S-056 | `rewrite_duckdb_view_sql` uses byte length for char-based offset (ASCII-only today) | `schema.rs:166-192` | S | **[FALSE POSITIVE]** code already uses char-based iteration |
| R5-S-057 | `delete_filter.rs` u32 indices limit batch to 4B rows (no guard) | `delete_filter.rs` | S | **[VERIFIED]** guard already existed |
| R5-S-058 | `extract_update_info` silently skips extra projection expressions | `query_planner.rs:202-205` | S | **[FIXED]** fix-dml-robustness |
| R5-S-059 | `schema_names()` swallows errors with `unwrap_or_default` (DF trait limitation) | `catalog.rs:344-354` | S | **[FIXED]** fix-backend-parity |
| R5-S-060 | `deregister_schema` cascade creates multiple snapshots | `catalog.rs:242-258` | S | **[SKIPPED]** requires MetadataWriter trait API change |
| R5-S-061 | `build_inlined_data_exec` column lookup is O(n²) per row | `table.rs` | S | **[FIXED]** fix-write-safety |
| R5-S-062 | Compaction SQL string interpolation — defense-in-depth note (currently safe) | `compaction_functions.rs` | S | **[VERIFIED]** SQL interpolation already safe |

#### Interop Nits

| ID | Description | Files | Effort |
|----|-------------|-------|--------|
| R5-S-063 | UUID v4 (ours) vs v7 (DuckDB) — cosmetic | `table_writer.rs:85`, DML exec plans | S | **[VERIFIED]** cosmetic only, no change |
| R5-S-064 | MERGE snapshot_changes always records both inserted+deleted regardless of actual ops | `merge_exec.rs:638-643` | S | **[FIXED]** fix-dml-robustness |
| R5-S-065 | MySQL `TIMESTAMP(6)` has Y2038 limit — use `DATETIME(6)` | `metadata_writer_mysql.rs:31` | S | **[FIXED]** fix-backend-parity |
| R5-S-066 | MySQL VARCHAR(255/1024) limits may truncate long paths | `metadata_writer_mysql.rs:22-324` | S | **[FIXED]** fix-backend-parity |
| R5-S-067 | Cross-engine tests only use SQLite backend, not PG/MySQL | `tests/cross_engine_tests.rs:48-68` | M | **[FIXED]** fix-cross-engine |
| R5-S-068 | Parquet v2.0 vs DuckDB default v1.0 — no functional issue | `table_writer.rs:176`, DML exec plans | — | **[SKIPPED]** no functional issue |

#### Test/Robustness Nits

| ID | Description | Files | Effort |
|----|-------------|-------|--------|
| R5-S-069 | DuckDbConn::query discovers columns by trial-and-error, may truncate on non-String types | `tests/common/test_utils.rs:414` | S | **[FIXED]** fix-test-infrastructure |
| R5-S-070 | No dedicated test for transaction-aware read routing in hybrid adapter | `tests/hybrid_asyncdb.rs:230-260` | S | **[FIXED]** fix-test-infrastructure |

#### Codex Miscellaneous

| ID | Description | Files | Effort |
|----|-------------|-------|--------|
| R5-S-071 | `set_data_path` non-atomic (DELETE then INSERT) in PG/MySQL — theoretical concurrent issue | `metadata_writer_postgres.rs:1160`, `metadata_writer_mysql.rs:1263` | S | **[FIXED]** fix-backend-parity |
| R5-S-072 | `initialize_schema` not atomic across backends — partial catalog state on failure | All writers | S | **[FIXED]** fix-backend-parity |
| R5-S-073 | DuckDB `table_exists`/`get_table_row_count` `unwrap_or(false/0)` suppresses DB errors | `metadata_provider_duckdb.rs:100`, `:602` | S | **[FIXED]** fix-metadata-correctness |
| R5-S-074 | `get_table_row_count` uses separate statements (file + inline count) without transaction | All providers | S | **[SKIPPED]** minimal impact |
| R5-S-075 | `join_paths` normalizes interior `//` — can rewrite valid object-store keys | `path_resolver.rs:277-279` | S | **[VERIFIED]** limitation documented |
| R5-S-076 | Delete file size `unwrap_or(0)` hides metadata errors | `table_deletions.rs:119-132` | S | **[FIXED]** fix-dml-robustness |
| R5-S-077 | `cross_engine_df_write_duckdb_read` doesn't check float `score` column | `tests/cross_engine_tests.rs:213` | S | **[FIXED]** fix-cross-engine |
| R5-S-078 | MySQL snapshot change format inconsistent (`"Altered table"` vs `"altered_table:"`) | `metadata_writer_mysql.rs:2410` | S | **[FIXED]** fix-backend-parity |
| R5-S-079 | `nulls_allowed` NULL→true coercion hides catalog corruption | All 4 providers | S | **[FIXED]** fix-backend-parity |
| R5-S-080 | `test_table_rewrite` uses `contains()` instead of exact SQL match | `tests/hybrid_asyncdb.rs:758` | S | **[FIXED]** fix-test-infrastructure |
| R5-S-081 | `cross_engine_null_handling` test only checks 2 cells + row count | `tests/cross_engine_tests.rs:457` | S | **[FIXED]** fix-cross-engine |
